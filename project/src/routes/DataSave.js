    ...
    rows.push(
      <Row key={i} id={i}>
        ...
        <td>
          ...
          <ImgBtn type="file" id={`file${i}`} onChange={img => selectImg(img, i)}></ImgBtn>
        </td>
...
const inputChange = (e, hscodes, setHscodes, i) => {
  ...
  if (newcodes[i].length === 5) {
    if (newcodes[i][4] !== '.') {
      newcodes[i] = newcodes[i].substring(0, 4) + '.' + newcodes[i].substring(4);
    } else {
      newcodes[i] = newcodes[i].substring(0, 4);
    }
  } else if (newcodes[i].length === 8) {
    if (newcodes[i][7] !== '-') {
      newcodes[i] = newcodes[i].substring(0, 7) + '-' + newcodes[i].substring(7);
    } else {
